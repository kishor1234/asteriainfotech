...
$sql = $main->selectCount("price", "id"); // . $main->whereSingle(array("isActive" => "0"));
        ...
        <div class="form-group" id='adp'>
            <div class="col-lg-12" style="overflow: auto;">
                <table id="example" class="table table-hover  table-responsive table-bordered table-striped">
                    ...
                    <?php
                    $sql = $main->select("price", $_SESSION["db_1"]) . $main->orderBy("DESC", "id") . $main->limitWithOffset($offset, $limit);
                    $result = $main->adminDB[$_SESSION["db_1"]]->query($sql);
                    $i = 0;
                    while ($row = $result->fetch_assoc()) {
                        ...
                        <tr id="#trsk<?php echo $row["id"]; ?>">
                            ...
                            <td><a href="javascript:void(0)" onclick="return deleteLogo('<?php echo $obj->encdata("C_DeletePrise"); ?>', '#msg', '<?php echo $row["id"]; ?>')" class="btn btn-danger btn-sm" title="Delete"><i class="fa fa-trash"></i>  </a>
                                <a href="/?r=<?php echo $obj->encdata("C_OpenLinkTrue")."&v=".$obj->encdata("VEditPrice")."&i=".$row["id"];?>" class="btn btn-primary btn-sm" title="Edit"><i class="fa fa-edit"></i>  </a></td>
                        </tr>
                        <?php
                    }
                    ?>
