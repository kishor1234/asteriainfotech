...
$sql = $main->selectCount("slider", "id"); // . $main->whereSingle(array("isActive" => "0"));
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <form action="javascript:void(0)" id="upCertificate" onsubmit="return postDataWithRedirect('<?php echo $obj->encdata("C_UploadSlider"); ?>', '#upCertificate', '#msg', '<?php echo HOSTURL . "?r=" . $obj->encdata("C_OpenLink") . "&v=" . $obj->encdata("VSliders"); ?>')" class="form-horizontal"   method="post" enctype="multipart/form-data">
                            <table class="table">
                                <tr>
                                    
                                    <td>
                                        <label>Image file <span class="text-danger">*</span></label>
                                        <input type="file" required=""  id="imagefile" name="imagefile" placeholder="PDF Certificate" accept="image/*" /> 
                                    </td>
                                </tr>
                                <tr>
                                    <td>
                                        <label>Slider Information <span class="text-danger">*</span></label>
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <input list="slidecategory" name="category" class="form-control" id="category" required>
                                        <datalist id="slidecategory">
                                            <?php
                                            $sql=$main->select("post",$_SESSION["db_1"]).$main->whereSingle(array("category"=>"Page"));
                                            $result=$main->adminDB[$_SESSION["db_1"]]->query($sql);
        ...
        <div class="form-group" id='adp'>
            <div class="col-lg-12" style="overflow: auto;">
                <table id="example" class="table table-hover  table-responsive table-bordered table-striped">
                    ...
                    <?php
                    $sql = $main->select("slider", $_SESSION["db_1"]) . $main->orderBy("DESC", "id") . $main->limitWithOffset($offset, $limit);
                    $result = $main->adminDB[$_SESSION["db_1"]]->query($sql);
                    $i = 0;
                    while ($row = $result->fetch_assoc()) {
                        $i++;
                        ...
                        <tr id="#trsk<?php echo $row["id"]; ?>">
                            ...
                            <td><img src="<?php echo $row["img"]; ?>" style="width:50px; height:auto;"></td>
                            <td><?php echo $row["info"]; ?></td>
                            <td><?php echo $row["category"]; ?></td>
                            <td><?php echo $row["ip"]; ?></td>
                            <td><?php echo $row["onCreate"]; ?></td>
                            <td>
                                <a href="javascript:void(0)" onclick="return deleteLogo('<?php echo $obj->encdata("C_DeleteSlider")."&p=".$row["path"]; ?>', '#msg', '<?php echo $row["id"]; ?>')" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i></a>
                            </td>
                        </tr>
                        <?php
                    }
