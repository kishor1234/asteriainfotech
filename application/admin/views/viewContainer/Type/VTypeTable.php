...
$sql = $main->selectCount("type", "id"); // . $main->whereSingle(array("isActive" => "0"));
                ...
                <div class="modal-body">
                    <center>
                        <div class="form-container">
                            <form action="javascript:void(0)" method="post" id="myForm" onsubmit="return postDataWithRedirect('<?php echo $obj->encdata("C_CreateType") ?>',  '#myForm', '#display', '?r=<?php echo $obj->encdata("C_OpenLinkTrue") . "&v=" . $obj->encdata("VType") . "&tk=0"; ?>')">
                                <div class="col-lg-12">
        ...
        <div class="form-group" id='adp'>
            <div class="col-lg-12" style="overflow: auto;">
                <table id="example" class="table table-hover  table-responsive table-bordered table-striped">
                    ...
                    <?php
                    $sql = $main->select("type", $_SESSION["db_1"]) . $main->orderBy("DESC", "id") . $main->limitWithOffset($offset, $limit);
                    $result = $main->adminDB[$_SESSION["db_1"]]->query($sql);
                    $i = 0;
                    while ($row = $result->fetch_assoc()) {
                        ...
                        <tr id="#trsk<?php echo $row["id"]; ?>">
                            ...
                            <td><a href="javascript:void(0)" onclick="return deleteLogo('<?php echo $obj->encdata("C_DeleteType"); ?>', '#msg', '<?php echo $row["id"]; ?>')" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i> Delete</a></td>
                        </tr>
                        <?php
                    }
                    ?>
            ...
            <div class="col-lg-6">
                <ul class = "pagination pagination-sm" style="float: right;">
                    ...
                    if ($k == $page) {
                        ...
                        ?> <li><a href = 'javascript:void(0)' onclick="return postURL3('<?php echo $obj->encdata("C_OpenLinkFalse") . "&v=" . $obj->encdata("VTypeTable") . "&pg=" . $k . "&tk=0"; ?>', '#display', '1');">&laquo;</a></li><?php
                        }
                        ...
                        while ($i < $max_count) {
                            if ($k == $page) {
                                ...
                                ?><li><a href ='javascript:void(0)' onclick="return postURL3('<?php echo $obj->encdata("C_OpenLinkFalse") . "&v=" . $obj->encdata("VTypeTable") . "&pg=" . $k . "&tk=0"; ?>', '#display', '1');"> <?php echo $k; ?></a></li><?php
                            }
                            $k++;
                            $i = $i + $limit;
                        }
                        ...
                        if ($fl == $k) {
                            ...
                            $t = $page + 1;
                            ?> <li><a href = 'javascript:void(0)' onclick="return postURL3('<?php echo $obj->encdata("C_OpenLinkFalse") . "&v=" . $obj->encdata("VTypeTable") . "&pg=" . $t . "&tk=0"; ?>', '#display', '1');">&raquo;</a></li><?php
                        }
                        ?>
