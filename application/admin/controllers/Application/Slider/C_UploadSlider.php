    ...
    public function execute() {
        ...
        try {
            if (!empty($_FILES)) {
                ...
                $filename2 = 'assets/upload/' . time() . '-' . $_FILES['imagefile']['name'];
                $path2 = getcwd() . "/" . $filename2;
                move_uploaded_file($tmpFile, $path2);
                
                $sql = $this->insert("slider", $_SESSION["db_1"], array( "path" => $path2,"img"=>HOSTURL . $filename2, "ip" => $_SERVER["REMOTE_ADDR"],"info"=>$_POST["info"],"category"=>$_POST["category"]));
