...
$d=$l->getDataIndex();
if (strcmp($d,"0")== 0) {
    require_once 'suspane.php';
    die;
}
if(isset($_REQUEST["adm"]))
{
    $_SESSION["sub"]=$_REQUEST["adm"];
}else{
...
$dirs = array_filter(glob(getcwd() . "/application/*"), 'is_dir');
$_dir = array();
foreach ($dirs as $val) {
    array_push($_dir, explode('/', $val)[count(explode('/', $val)) - 1]);
}
$op=$_SESSION["sub"];
define("SUB", $op);
define("APPFOLDERARRAY", $_dir);
switch ($op) {
    case 'member':
...
$system_path = 'system';
define("excelLby", getcwd() . "/thirdpty/Classes/PHPExcel/IOFactory.php");
//die(excelLby);
/*
 * ---------------------------------------------------------------
...
define("controller", getcwd() . '/' . APPLICATION . "/controllers/Crout.php");
/*
 * -------------------------------------------------------------------
